...
function initUnlockButton(triggerImg) {
  if (triggerImg.__unlockButtonInit) return;
  triggerImg.__unlockButtonInit = true;

  console.log("Unlock button payload started");

  function addButton() {
    if (triggerImg.nextElementSibling?.classList.contains("unlock-btn-wrapper")) {
      return; // already there
    }

    const wrapper = document.createElement("div");
    wrapper.className = "unlock-btn-wrapper";
    ...
    if (!btn.dataset.bound) {
      ...
      btn.addEventListener("click", unlockClickHandler);
    }
  }

  function unlockClickHandler(e) {
    e.stopPropagation();
    e.preventDefault();
  ...
  // Keep it alive through rerenders
  const obs = new MutationObserver(() => addButton());
  obs.observe(triggerImg.parentNode || document.body, {
    childList: true,
    subtree: true
  });
}
